...
-- =====================================================================
PROMPT --- Tạo OLS Policy HOSPITAL_POLICY_2026 ---.

-- Xóa policy cũ nếu tồn tại
BEGIN
  SA_SYSDBA.DROP_POLICY(
    policy_name  => 'HOSPITAL_POLICY_2026',
    drop_column  => TRUE
  );
...
PROMPT --- Đã tạo HOSPITAL_POLICY_2026 ---.
...
BEGIN SA_COMPONENTS.CREATE_LEVEL('HOSPITAL_POLICY_2026', 30, 'BGD',      'Ban Giam Doc - Cap cao nhat');      EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN SA_COMPONENTS.CREATE_LEVEL('HOSPITAL_POLICY_2026', 20, 'LDKHOA',   'Lanh dao Khoa');                    EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN SA_COMPONENTS.CREATE_LEVEL('HOSPITAL_POLICY_2026', 10, 'NHANVIEN', 'Nhan vien thuong');                 EXCEPTION WHEN OTHERS THEN NULL; END;
/
...
BEGIN SA_COMPONENTS.CREATE_COMPARTMENT('HOSPITAL_POLICY_2026', 100, 'TIEUHOA',  'Khoa Tieu Hoa');  EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN SA_COMPONENTS.CREATE_COMPARTMENT('HOSPITAL_POLICY_2026', 200, 'THANKINH', 'Khoa Than Kinh'); EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN SA_COMPONENTS.CREATE_COMPARTMENT('HOSPITAL_POLICY_2026', 300, 'TIMMANH',  'Khoa Tim Manh'); EXCEPTION WHEN OTHERS THEN NULL; END;
/
...
BEGIN SA_COMPONENTS.CREATE_GROUP('HOSPITAL_POLICY_2026', 10, 'HCM',      'TP. Ho Chi Minh', NULL);     EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN SA_COMPONENTS.CREATE_GROUP('HOSPITAL_POLICY_2026', 20, 'HAIPHONG', 'Hai Phong',       NULL);     EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN SA_COMPONENTS.CREATE_GROUP('HOSPITAL_POLICY_2026', 30, 'HANOI',    'Ha Noi',          NULL);     EXCEPTION WHEN OTHERS THEN NULL; END;
/
...
-- → Bao gồm tất cả compartments và groups để đọc được mọi thông báo
-- -----------------------------------------------------------------------
BEGIN
...
BEGIN SA_LABEL_ADMIN.CREATE_LABEL('HOSPITAL_POLICY_2026', 1001, 'NHANVIEN',                         TRUE); EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN SA_LABEL_ADMIN.CREATE_LABEL('HOSPITAL_POLICY_2026', 1002, 'BGD',                               TRUE); EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN SA_LABEL_ADMIN.CREATE_LABEL('HOSPITAL_POLICY_2026', 1003, 'LDKHOA',                            TRUE); EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN SA_LABEL_ADMIN.CREATE_LABEL('HOSPITAL_POLICY_2026', 1004, 'LDKHOA:TIEUHOA',                    TRUE); EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN SA_LABEL_ADMIN.CREATE_LABEL('HOSPITAL_POLICY_2026', 1005, 'NHANVIEN:TIEUHOA:HCM',              TRUE); EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN SA_LABEL_ADMIN.CREATE_LABEL('HOSPITAL_POLICY_2026', 1006, 'NHANVIEN:TIEUHOA:HANOI',            TRUE); EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN SA_LABEL_ADMIN.CREATE_LABEL('HOSPITAL_POLICY_2026', 1007, 'LDKHOA:TIEUHOA,THANKINH:HAIPHONG',  TRUE); EXCEPTION WHEN OTHERS THEN NULL; END;
/
...
UPDATE "THÔNGBÁO"
SET OLS_LABEL = CHAR_TO_LABEL('HOSPITAL_POLICY_2026', 'NHANVIEN')
WHERE "MÃTB" = 1;

-- t2 (MÃTB=2): BGD - chỉ Ban Giám Đốc
UPDATE "THÔNGBÁO"
SET OLS_LABEL = CHAR_TO_LABEL('HOSPITAL_POLICY_2026', 'BGD')
WHERE "MÃTB" = 2;

-- t3 (MÃTB=3): LDKHOA - lãnh đạo khoa trở lên
UPDATE "THÔNGBÁO"
SET OLS_LABEL = CHAR_TO_LABEL('HOSPITAL_POLICY_2026', 'LDKHOA')
WHERE "MÃTB" = 3;

-- t4 (MÃTB=4): LDKHOA:TIEUHOA - lãnh đạo khoa Tiêu hóa
UPDATE "THÔNGBÁO"
SET OLS_LABEL = CHAR_TO_LABEL('HOSPITAL_POLICY_2026', 'LDKHOA:TIEUHOA')
WHERE "MÃTB" = 4;

-- t5 (MÃTB=5): NHANVIEN:TIEUHOA:HCM - nhân viên Tiêu hóa tại HCM
UPDATE "THÔNGBÁO"
SET OLS_LABEL = CHAR_TO_LABEL('HOSPITAL_POLICY_2026', 'NHANVIEN:TIEUHOA:HCM')
WHERE "MÃTB" = 5;

-- t6 (MÃTB=6): NHANVIEN:TIEUHOA:HANOI - nhân viên Tiêu hóa tại Hà Nội
UPDATE "THÔNGBÁO"
SET OLS_LABEL = CHAR_TO_LABEL('HOSPITAL_POLICY_2026', 'NHANVIEN:TIEUHOA:HANOI')
WHERE "MÃTB" = 6;

-- t7 (MÃTB=7): LDKHOA:TIEUHOA,THANKINH:HAIPHONG - lãnh đạo Tiêu hóa+Thần kinh tại Hải Phòng
UPDATE "THÔNGBÁO"
SET OLS_LABEL = CHAR_TO_LABEL('HOSPITAL_POLICY_2026', 'LDKHOA:TIEUHOA,THANKINH:HAIPHONG')
WHERE "MÃTB" = 7;
...
SELECT
  ...
  COLUMN_NAME
FROM SA_POLICIES
WHERE POLICY_NAME = 'HOSPITAL_POLICY_2026';
